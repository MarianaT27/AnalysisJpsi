...
int tagged(string nameFile="PASS1_FALL", int version=-19, double Beam_E=10.2, int PASS=1, int max=0, int train=0, string ext="") {
    // Record start time
   ...
   // int S19_P2[11]  =   {6712, 6716, 6769};
    ...
    //RESULTS RESULTS
                          TH1F* h_Invariant= new TH1F("h_im","M(e+e-) ",100,2.5,3.5);
    ...
    //Start
    for(int fc =1; fc<10; fc++) {//Run 5032 to 5419 // 6616 6783
                    ...
                    if(electronAccepted && positronAccepted){
                        ...
                        if(abs(electron_vt-positron_vt)<=1){
                            ...
                            if(N_FT==1){
                                ...
                                //MM vs IM
                                if((electron.vtime-FT_vt)<=3.5 && (electron.vtime-FT_vt)>=-3.5){//1,2
   ...
   can->Print( (pdf_original + "(").c_str());
    ...
    h_MM->Draw();
    TF1* fit=Fit_Gauss(h_MM,0.3,1.5);
    /*TF1 *func = new TF1("fitf",fitf,0.6,1.4,3);
    func->SetParameters(300,0.938,0.5);
    func->SetParNames("Constant","Mean_value","Sigma");
    h_MM->Fit(func,"r");*/

    //int bin1 = h_MM->FindFirstBinAbove(h_MM->GetMaximum()/2);
    //int bin2 = h_MM->FindLastBinAbove(h_MM->GetMaximum()/2);
    //double fwhm = fit->GetParameter(2)*sqrt(8*log(2));
    double fwhm = fit->GetParameter(2)*2.355;
    TPaveText *pt_fit = new TPaveText();
    pt_fit->AddText(Form("FWHM :%f ",fwhm));   
    pt_fit->SetTextSize(0.03);
    pt_fit->Draw();
    can->Print( (pdf_original + "(").c_str());
    ...
    h_Invariant->Draw();
    Fit_func.Fit_CrystallBall_Pol2(h_Invariant);
    //pt->AddText((fitnames[1]).c_str());   
    //pt->SetTextSize(0.03);
    //pt->Draw();
